...
export function InfoTutorials() {
  const [tutorialsList, setTutorialsList] = useState<IInfoTutorials>({
    syllabus: [],
    description: [],
  });
  ...
  return (
    <>
      {tutorialsList?.syllabus.length && (
        <div>
          {tutorialsList.syllabus.map((item: any, index: any) => (
            <div key={index}>
              <div
                className={` ${syllabusId === item.id ? " border-blue-200 " : ""} bg-white border border-transparent rounded-xl  `}
              >
                <button
                  onClick={() => {
                ...
                <div
                  className={` ${syllabusId === item.id ? "block" : "hidden"} w-full overflow-hidden transition-[height] duration-300`}
                >
                  <div className="pb-4 px-5">
                    {tutorialsList.description
                      .filter((desc) =>
                        desc?.id ? accordion?.includes(desc?.id) : false,
                      )
                      .map((item, index) => (
                        <div
                          key={index}
                          ...
                          <div className="flex items-center gap-x-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-gray-500">
                                ...
                                adipisicing elit. Quos, voluptatum?
                              </span>
                            </div>
                          </div>
