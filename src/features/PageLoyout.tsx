...
export function PageLayout({
  ...
  useEffect(() => {
    ...
    //  ?  get previous lunch data
    const getV5LaunchesPrevious = async () => {
      // ? post request
      const payload = {
        query: {
          upcoming: false,
    ...
    // ? get data
    getV5LaunchesPrevious()
      .then((data) => dispatch(updatePreviousLaunches(data)))
      .catch((error) => console.log(error));
    getV5Launches()
      .then((data) => dispatch(updateUpcomingLaunches(data)))
      .catch((error) => console.log(error));
  }, [dispatch]);
          ...
          <div className=" flex flex-col gap-3">
            {
              routesLink.map((route, index) => (
                <a
                  className={`${route.path !== window.location.pathname
                    ? ""
                    : "font-semibold"
                    } text-2xl ease-out duration-200 hover:bg-gray-100 rounded-xl text-[rgb(53, 53, 53)]`}
                  key={index}
                  href={route.path}
                >
                  {route.main()}
        ...
        >
          <div className=" w-full lg:w-1/5 hidden lg:block px-8">
            <div className=" py-10 px-2  max-w-64">
              <a href="/" className="sc-bdfBQB sc-jrAFXE jTSIYX DZVSn">
                <svg
                  viewBox="0 0 201 50"
                  fill="none"
                ...
                {routesLink.map((route, index) => (
                  <a
                    className={`${route.path !== window.location.pathname
                      ? ""
                      : "font-semibold"
                      } text-2xl ease-out duration-200 hover:bg-gray-100 rounded-xl py-2 px-2 text-[rgb(53, 53, 53)]`}
                    key={index}
                    href={route.path}
                  >
                    {route.main()}
          ...
          >
            <div className=" flex justify-between items-center p-6 lg:hidden">
              ...
              <div>
                <MdMenu
                  size={40}
                  onClick={() => dispatch(updateIsSidebarOpen(true))}
